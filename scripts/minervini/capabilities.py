...
CAPABILITIES = {
    ...
    for item in [
        ...
        _capability(
            "health",
            "Check deterministic runtime readiness without making an investment judgment or network request.",
            inputs=_inputs(),
            output="Python, doctrine-registry, yfinance, and exact ibd-rs-rating version readiness.",
            limitations=["This checks installed dependencies and local doctrine only; it does not prove that remote providers are reachable.", "--as-of stamps the envelope but does not alter dependency readiness."],
            status_meanings={"ok": "All required local components are ready.", "partial": "At least one required local component is missing or mismatched."},
            examples=["scripts/.venv/bin/python scripts/pipeline health"],
        ),
        _capability(
            "clock",
        ...
        _capability(
            ...
            output="The executable claim plus separated provenance metadata.",
            limitations=["This is runtime doctrine, not a full book transcript or citation browser.", "--as-of stamps the envelope; doctrine content is versioned with the repository rather than market time."],
        ...
        _capability(
            ...
            limitations=["QQQ versus 21 EMA is context only and cannot authorize risk-on by itself.", "Finviz breadth is a current completed-session snapshot and cannot reconstruct an uncaptured historical page.", "A missing source remains unavailable; no web or proxy number replaces it."],
            status_meanings={"ok": "All requested evidence is available, including trade traction.", "partial": "Some independent sources or breadth sections are unavailable.", "needs_input": "Trade traction is absent even though provider evidence may exist.", "unavailable": "No market evidence source succeeded."},
        ...
        _capability(
            ...
            output="Eligible instruments, structured exclusions, and pagination metadata including universe-wide recommendation count.",
            limitations=["Nasdaq Trader supplies a current security master, not historical constituents; unsupported historical requests are unavailable.", "ETF, SPAC, shell, OTC, preferred, warrant, and unsupported instruments are excluded with reasons.", "Candidate eligibility is not itself a buy recommendation."],
        ...
        _capability(
            ...
            limitations=["All eight standard criteria are AND conditions; fundamentals and narrative cannot waive a known failure.", "RS comes only from ibd-rs-rating 0.5.0 and is never approximated.", "A chart judgment is required for ambiguous Primary Base quality."],
            status_meanings={"ok": "Available evidence produced eligible, avoid, or incomplete domain state.", "partial": "Price evidence succeeded but RS or another independent source is unavailable.", "unavailable": "Completed price history is unavailable.", "needs_input": "A supplied argument is invalid."},
        ...
        _capability(
            ...
            status_meanings={"ok": "The setup reducer produced ready, wait, or avoid.", "needs_input": "Required chart evidence remains incomplete or needs_chart.", "unavailable": "Completed price history is unavailable."},
        ...
        _capability(
            ...
            inputs=_inputs(
                {
                    ...
                    "stop_effective_date": _field("date", "First completed session governed by --stop-price in active mode; defaults to --entry-date. Supply the actual date when a raised or replaced stop became effective."),
                    ...
                    "invalidation_price": _field("number", "Positive structural invalidation price."),
                    "invalidation_condition": _field("string", "Observable structural invalidation condition."),
            ...
            output="The sole final ticker verdict with component states, completed stop-path evidence, failed/waiting/missing evidence, stop constraints, reward-to-risk, and 3R protection context.",
            ...
            limitations=["This capability never recommends portfolio allocation or position size.", "A hard stop may not exceed 10% or half the supplied realized average gain; reward-to-risk must be at least 2:1.", "A completed current price above the stop cannot establish HOLD: the provider must cover every completed daily Low from --stop-effective-date through --as-of; a recovered price does not erase an earlier breach.", "--stop-effective-date defaults to --entry-date. When a stop was raised or replaced later, supply its actual effective date so the newer stop is not applied retroactively.", "An explicit --current-price at or below the stop can trigger SELL; a partial-session breach must instead use the explicit live-stop flags.", "A live stop breach triggers SELL only when --live-stop-check is explicit; ordinary gates use completed bars.", "The reducer does not infer unsupported market, eligibility, setup, or fundamentals states."],
            status_meanings={"ok": "A complete BUY-READY, WAIT, AVOID, HOLD, or SELL verdict was produced; active HOLD includes a clear completed stop path.", "partial": "The completed-price provider failed or could not cover the full active stop window.", "needs_input": "Required evidence is missing, so the domain verdict is INCOMPLETE.", "unavailable": "An internal required capability cannot be evaluated."},
        ...
        _capability(
            ...
            status_meanings={"ok": "Both chart artifacts and the manifest were written.", "unavailable": "Completed price history is unavailable.", "needs_input": "The destination path or ticker is invalid."},
