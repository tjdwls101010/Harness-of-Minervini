            ...
            inputs=_inputs(
                {
                    ...
                    "chain_completeness": _field("enum", "Whether the declared chain is the base's whole structure. A caller may say partial, which admits a gap. A caller may not say complete: the reading exists to check the chain, and the chain's own author cannot perform that check, so complete counts only when completeness_source names an independent segmentation. Until one exists the standard route stops at wait.", choices=["complete", "partial", "needs_chart"]),
                    "completeness_source": _field("enum", "Who supplied the completeness reading. Only an independent segmentation can vouch for a chain the caller declared.", choices=["independent_segmentation"]),
                    "entry_proximity": _field("enum", "Chart reading of how close the entry sits to the pivot. The source says to buy as close to the pivot as possible without chasing more than a few percentage points and supplies no number, so the call is the reader's; the distance now, the distance at the breakout, and the breakout's age are reported beside it.", choices=["at_pivot", "chased", "needs_judgment"]),
