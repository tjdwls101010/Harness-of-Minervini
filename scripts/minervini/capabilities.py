            ...
            inputs=_inputs(
                {
                    ...
                    "entry_proximity": _field("enum", "How close the entry sits to the pivot. The source says to buy as close to the pivot as possible without chasing more than a few percentage points and supplies no number, so the call is the reader's outright: the only reading the bars refuse is at_pivot on a pivot price has not cleared. The distance now, the distance at the breakout, the breakout's age, and Minervini's own stated five-to-twenty-cent buffer are reported for the reader to make the call with.", choices=["at_pivot", "chased", "needs_judgment"]),
