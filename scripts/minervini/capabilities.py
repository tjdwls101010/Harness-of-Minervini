...
CAPABILITIES = {
    ...
    for item in [
        ...
        _capability(
            ...
            inputs=_inputs({"ticker": _field("string", "US-listed ticker symbol.", required=True)}, providers=True),
            output="The advance, the flag and the volume behind them, each of the source's four criteria read against what was measured, and whether the structure is out, unresolved, or clear on everything the bars can settle.",
            limitations=[
                ...
                "A marginal new high inside a tight flag restarts the structure, because nothing in the source says how large a new high has to be before it means something. The bars are therefore read from every top the search could have landed on -- the highest of the span, then the highest below that, down until the next top stands further below the highest than the distance registered as this harness's own convention for candidate tops -- not the sources' tight-action figure it happens to equal, which describes a flag's decline after a peak has been chosen and offers VCP character as an alternative to it. Past that distance a top is an older structure the stock has since overtaken rather than another reading of this one. The first top past the distance is reported with how far past it stands, so the boundary a contest was decided next to can be audited. A lower top the loaded history cannot reach behind is reported too, and withholds the rejection rather than ending the chain: it is a top nobody read, which is the shape a recently listed stock arrives in. The registered distance decides which of those tops may *contest* a criterion -- a top far below the highest is a different structure, and letting it dispute a limit would leave every criterion permanently open. It does not decide which may object: a rejection stands only where every top in the span was readable and reached one, so a structure the chain walked past still prevents a rejection, and so does a candidate whose own span holds a corporate action and could not be read at all. Two readings were not enough: two ticks a hundredth of a percent apart hand the search three tops, and both of the first two reject while the structure they sit inside has nothing decisive against it. Across every cached history the chain runs one to fifteen tops and all twenty-three still reject on a criterion every reading agreed on.",
                "Cash distributions are measured, not corrected either, and unlike a split the amount is known -- so a payout withholds only the criteria it actually decided rather than the whole reading. A flag printing thirty-six percent that is twenty-four once the payout is added back was decided by the payout; an ordinary quarterly payment is a fraction of a percent against a twenty-five percent limit and decides nothing. Only what was paid on or before the flag's low counts against that low: a payout made after the flag had already bottomed took the later prints down without having taken the low down. What a payout can also do is choose the peak, because a session before an ex-date can outprint the top the stock actually made and the real top is then later than the one the chain starts from -- so the tops are re-found on a series with every print on one scale and the whole structure's boundaries are compared, not just the peak -- a payout can leave the top exactly where it was and still move where the advance started, how long it took and which forty sessions the volume was measured against. Any of those moving leaves nothing here deciding. A history that does not carry the distribution column has not said there were none, and that absence is its own gap, the same as a missing split column. What is not done is re-measuring the structure in adjusted coordinates: the peak, the anchor, the durations and the volume spans stay on the prints the tape made.",
                ...
                "The fingerprint covers the five price columns and not the corporate actions, so two inputs with identical bars and different split events digest the same. That is why no chart approval is accepted here yet: an approval bound to that digest would not be bound to the evidence the verdict actually turns on.",
            ],
            status_meanings={"ok": "The bars settled it: at least one criterion the source states failed on measurement.", "partial": "Nothing measurable failed, and the structure is not qualified either -- a chart reading, corporate-action evidence, or both are still missing, and each gap names its own cause.", "unavailable": "Completed price history is unavailable, or the history is unusable as bars at all -- the reason names which, in the same vocabulary ticker.swings and ticker.chart use."},
            examples=["scripts/.venv/bin/python scripts/pipeline ticker power-play AAOI", "scripts/.venv/bin/python scripts/pipeline ticker power-play NVDA --as-of 2026-06-19"],
