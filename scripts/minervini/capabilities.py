...
CAPABILITIES = {
    ...
    for item in [
        ...
        _capability(
            ...
            limitations=["Swing segmentation is the caller's chart reading; what the bars can check is that each declared date exists, that the chain alternates and ends on a high, and that each bar really is the extreme of its span.", "Ready requires every item of a route's declared evidence, so a base nobody described is incomplete rather than unobjectionable.", "Contrast evidence from other practitioners is reported beside the verdict and never enters it.", "The breakout is dated at the start of the run price is currently in, not at the first close above the pivot ever, and earlier attempts at the pivot are counted beside it.", "A chain declared from a late high is not a misread swing and is not refused; what it leaves above the entry is measured instead and reported as overhead supply.", "A cheat entry is not measurable yet: it is entered inside the base rather than at its pivot, so it returns incomplete naming the cheat geometry it needs rather than borrowing the pivot breakout's evidence.", "TL early requires opt-in, explicit confirmation debt, a later pivot, and precise invalidation, and it does not waive the supply gates, which are about the base rather than about when the trade is taken."],
