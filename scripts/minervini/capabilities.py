    ...
    for item in [
        ...
        _capability(
            ...
            limitations=[
                ...
                "A marginal new high inside a tight flag restarts the structure, because nothing in the source says how large a new high has to be before it means something. The bars are therefore read twice, from the highest top and from the highest top below it, and the criteria the two readings answer differently are reported as contested and decide nothing. They are reported per criterion rather than as one verdict: across every cached history the readings differed for ten of twenty-three tickers and all ten rejected under both readings, so withholding the whole verdict would have discarded a finding the bars made twice. What the second reading cannot cover is a third top, or an advance so smooth that the session before the peak becomes the rival -- resolving those needs a top defined by swing structure rather than by highest print, and the peak-identity gap is reported rather than resolved here.",
