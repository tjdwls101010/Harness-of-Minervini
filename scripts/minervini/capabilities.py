...
CAPABILITIES = {
    ...
    for item in [
        ...
        _capability(
            ...
            output="Eligibility route and state, exact Trend Template signals, completed-history depth, RS date/value, missing evidence, and next capabilities. Each criterion's limit and its required-value text are read from the doctrine registry, so the number a verdict used and the number it reports are the same one. `bands` carries any measurement the source stated as a range, with its measured value, the range, where the measurement sat against it, and which edge of it is the good one. Landing outside a range is not by itself a failure: a base shallower than a depth range is outside it on the favourable side, and `direction` beside `band_position` is what says so.",
            ...
            inputs=_inputs(
                {
                    ...
                    "measured_bars": _field(
                        "string",
                        "The digest ticker.chart reports beside the overlay, in power_play.measured_bars, and that every chart question carries in measured_bars. Required whenever chart_readings is supplied, alongside drawn_bars and checked the same way. Two digests rather than one because the picture and the span drawn on it do not have the same input: drawn_bars covers the five price columns, which identifies the candles, while this capability reads corporate actions as well -- a split inside a span leaves it deciding nothing, a payout withholds the criteria it decided -- so a history with identical prices and a different action column issues different questions under the same drawn_bars, and a chart drawn from it can carry no span at all while its digest matches. A mismatch is the same outcome a drawn_bars mismatch gets: nothing is applied, and approval_covers_different_bars comes back with measured_bars beside it to re-read from. Null where the history never said whether a split occurred, which is where no question is issued either.",
                    ),
                    "chart_readings": _field(
                        "string[]",
                        "Repeatable KEY=observed|absent, one per question this capability asked. The key comes from chart_questions and names the bars, the criterion, the reading of the tops and the measurement together, so an answer read from these bars and matching no open question is refused rather than dropped -- the ordinary way one goes stale is a session closing between the chart and the request. An answer read from another vintage is not refused for its keys, because this run issued none of them; nothing is applied and drawn_bars is what comes back. Answers live in the request and nowhere else, so every answer still wanted has to arrive in the same call: a second call carrying only the newly answered key silently loses the first call's, and the question it closed comes back open. Observed satisfies the criterion for the reading it was asked under and absent settles it against the stock for that same reading -- a top that may contest it goes on holding the criterion open until its own key is answered. A reader who could not tell supplies nothing.",
                    ),
                },
                providers=True,
            ),
            output="The advance, the flag and the volume behind them, each of the source's four criteria read against what was measured, and whether the structure is out, unresolved, or clear on everything the bars can settle. Two digests travel with it and mean different things: measured_bars covers the prices and the split and dividend events together and names the input the verdict was reached on, while measured_from is the price-only bars_fingerprint that ticker.chart also prints, and is what a reader compares against the picture in front of them. chart_questions lists what is still being asked -- one entry per criterion per reading of the tops, each with the key that answers it, the span it is asked about, the measurement it turns on, drawn_bars, and answered, which is the word supplied or null. A finished verdict keeps only the answered entries, because a rejection and a qualification both mean nothing is outstanding. reading_rejections lists the tops that were read and thrown out, each carrying the same span a question carries and the criteria it failed -- thrown out by the bars, or by a reader's own absent on a criterion the bars declined to decide. ticker.chart draws nothing for one the bars threw out, because an overlay a reader can see is one they can approve from and no key exists that would close a rejection. One a reader's own answer produced is a different case: that capability holds no answers, so it never sees the rejection and draws the span as it would have before. The span travels with both and the picture with only one, so a rejection the bars reached is read from the dates and measurements the rejection itself carries, against the whole-history panels that chart writes whatever it draws no overlay for. An observation answered by a reader carries read_from_chart so a pass a person supplied never reads like one the numbers reached. Five lists say which criteria a top that may contest them is holding open and how: contested_criteria for a top that answered differently, awaiting_chart_under_another_top for one whose chart is unread, payout_decided_under_another_top for one whose answer a dividend decided, corporate_action_under_another_top for one whose own span holds an action, and rejected_under_another_top for one the bars already threw out. Only awaiting_chart_under_another_top closes on a chart. The last two consent to nothing even where their numbers match -- an action's numbers are arithmetic about the action, and a rejected reading was never asked -- and no key exists that a reader could close either with. unread_top names the candidate the loaded history ends before, with its distance below the highest, and unread_top_may_contest says whether that distance lets it withhold a qualification. peak_is_a_confirmed_turning_point says whether the segmentation confirms the top the structure hangs from -- false leaves every measured failure standing and withholds a qualification under peak_not_a_confirmed_turning_point, because a flag tighter than one day's ordinary range confirms no turning point and a structure hanging from a bar nothing confirmed is not one this harness has identified a top for. readings_cover_other_bars says an answer was read from another vintage and nothing was applied.",
            limitations=[
        ...
        _capability(
            ...
            output="The sole final ticker verdict with component states, completed stop-path evidence, failed/waiting/missing evidence, stop constraints, reward-to-risk, and 3R protection context. An active HOLD also carries management_actions -- what to do while holding, each naming its action (RAISE_STOP, REDUCE, or REVIEW), the claim it comes from, and the measurement behind it; the list is empty when nothing is due, and SELL and INCOMPLETE carry no actions because there is nothing left to manage or no established position to manage. Three R is measured from the highest completed High since the entry session (max_high_since_entry, with its date), not from the last close: a gain that was reached and given back is exactly what the rule protects, and the last close is only the floor of what was reached when no High is available. Every limit comes from the doctrine registry: a gate decides pass or fail, while a band such as the ordinary loss target reports the measurement, the source's range, where the measurement sat against that range, and which edge of it is the good one -- a stop tighter than the range is outside it on the favourable side.",
        ...
        _capability(
            "ticker.chart",
            "Render auditable weekly-first and daily chart artifacts, plus a span-only panel wherever there is Power Play structure to read, from the same completed bars used by deterministic analysis.",
            inputs=_inputs(
                {
                    "ticker": _field("string", "US-listed ticker symbol.", required=True),
