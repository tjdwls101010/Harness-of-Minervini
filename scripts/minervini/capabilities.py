    ...
    for item in [
        ...
        _capability(
            ...
            limitations=[
                ...
                "The retracement is the harness's own convention, not the source's, which describes swing reading as chart work and never names a percentage. It is registered as a parameter rather than a threshold and is not a caller input.",
                "A segmentation that neighbouring parameter values disagree with is not proposed at all: the disagreement is returned instead, because a chain that moves when the parameter moves half a point is not one to check anything against.",
            ...
            status_meanings={"ok": "A segmentation the detector will vouch for.", "needs_input": "Neighbouring parameter values disagree, or the history segments into nothing.", "partial": "Completed price history stops before the requested session.", "unavailable": "Completed price history is unavailable."},
        ...
        _capability(
            ...
            limitations=["Charts corroborate qualitative ambiguity but never override a deterministic hard gate.", "Weekly is rendered before daily; historical as-of is enforced before rendering.", "The detector's turning points and its pivot are marked on the price panel, because this is where a person turns that proposal into the approval ticker.setup asks for. A segmentation the detector will not vouch for draws nothing rather than showing a structure the engine refuses to use, and an anchor with no bar in the rendered timeframe is left off rather than placed on a session that does not exist."],
