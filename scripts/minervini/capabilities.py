            ...
            inputs=_inputs(
                {
                    ...
                    "approved_bars": _field("string", "The bars_fingerprint the chain was approved from, as ticker.swings and ticker.chart both report it. Required whenever chain_completeness is declared, because a chart reading is a reading of one picture: comparing the anchor dates alone let a chain approved from another vintage of the series vouch for this one, with every date matching while the pivot, the depths and the base the reader looked at had all moved. A value that does not match the bars being judged does not fail the base -- it comes back incomplete naming approval_covers_different_bars, with the current fingerprint beside it to re-approve from."),
