            ...
            inputs=_inputs(
                {
                    ...
                    "chain_completeness": _field("enum", "Chart reading of whether the declared chain is the base's whole structure. Every anchor is checked against the bars, but a chain that skipped or merged a contraction has anchors that are genuinely their spans' extremes, so no measurement separates it from an honest one. Without this the setup stays incomplete rather than accepting a segmentation nobody vouched for.", choices=["complete", "partial", "needs_chart"]),
