            ...
            inputs=_inputs(
                {
                    ...
                    "chart_readings": _field(
                        "string[]",
                        "Repeatable KEY=observed|absent, one per question this capability asked. The key comes from chart_questions and names the bars, the criterion, the reading of the tops and the measurement together, so an answer matching no open question is refused rather than dropped. Observed satisfies the criterion and absent settles it against the stock; a reader who could not tell supplies nothing.",
                    ),
                },
                providers=True,
            ),
            output="The advance, the flag and the volume behind them, each of the source's four criteria read against what was measured, whichever of them are still waiting on a reader with the key that would answer each, and whether the structure is out, unresolved, or clear on everything the bars can settle.",
            limitations=[
                ...
                "Two of the criteria are answered by a reader, and that channel is the only way a human sentence becomes a machine pass here. It is bound accordingly: the capability issues a key per open question and accepts an answer only against a key it issued, where the key is a digest of the composite fingerprint of the bars and their split and dividend events, the criterion, every boundary session of the reading of the tops it was asked under, and the measurement the criterion turns on. A caller assembling those four fields themselves could match the three they kept and miss the one that moved, so the key is issued rather than described. Two words, observed and absent, both admissible on the same terms -- resolving a question the numbers declined to answer is not overriding a number. A reader who looked and could not tell supplies nothing and the envelope goes on asking. An answer matching no open question is refused by name, because the ordinary way one goes stale is a session closing between the chart and the request.",
                "What no reading of a chart closes falls out of that binding rather than from a rule forbidding it. A flag short of twelve sessions has not failed, it has not finished, and no chart adds a session. Two candidate tops asking the same question about different spans each get their own key, so answering one leaves the criterion disputed and settling it costs a reader both charts. A span holding a corporate action, or a history that cannot say whether one happened, is issued no key at all -- a reader cannot be asked to corroborate price action the split manufactured. A reading the bars already threw out is issued no key either, because a visual opinion never overturns a deterministic failure and a key there would send a reader to draw a picture with nothing left to decide -- its criteria report rejected_before_a_chart_was_needed instead. That is where the seam sits today: measured on the completed session of 2026-08-24 across the twenty-four tickers this repository has provider history for, it asks zero questions, because twenty-three reject on measurement and the twenty-fourth carries a corporate action inside its span. Every answered signal carries read_from_chart, so a verdict a person supplied never reads like one the numbers reached.",
            ],
            status_meanings={"ok": "The evidence contract is complete: either the bars settled it against the stock, or every criterion including the ones a reader answered is satisfied. The state says which -- a qualified Power Play here waives only unavailable verified fundamentals and never eligibility, setup, market, integrity or risk.", "partial": "Nothing measurable failed, and the structure is not qualified either -- a chart reading, corporate-action evidence, or both are still missing, and each gap names its own cause.", "unavailable": "Completed price history is unavailable, or the history is unusable as bars at all -- the reason names which, in the same vocabulary ticker.swings and ticker.chart use."},
