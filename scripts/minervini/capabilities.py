    ...
    for item in [
        ...
        _capability(
            ...
            limitations=[
                ...
                "A marginal new high inside a tight flag restarts the structure, because nothing in the source says how large a new high has to be before it means something. The bars are therefore read from every top the search could have landed on -- the highest of the span, then the highest below that, down until the next top stands further below the highest than the tightness this criterion itself allows, which is where a top stops being a competing reading of one structure and becomes an older one the stock has since overtaken. A criterion decides only where every reading answers it the same way, and a rejection stands only where every reading reaches one. Two readings were not enough: two ticks a hundredth of a percent apart hand the search three tops, and both of the first two reject while the structure they sit inside has nothing decisive against it. Across every cached history the chain runs one to fifteen tops and all twenty-three still reject on a criterion every reading agreed on.",
