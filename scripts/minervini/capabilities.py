    ...
    for item in [
        ...
        _capability(
            ...
            limitations=[
                ...
                "The base's left edge is where the stock last broke out of a structure and stayed out -- a close above a prior high, before the pivot formed, on volume above its own preceding fifty sessions, with every low since above that high. Fewer than fifty prior sessions is not that observation, so no boundary is drawn there. Two limits are known and left standing rather than papered over: holding admits no reset, so a breakout followed by a shallow failure and an ordinary-volume recovery is a departure the source would recognise and this does not; and the high it is measured at is any confirmed high rather than a pivot whose base is separately established, so a lone volume expansion at an interior rally the stock never returned under can end a base early. Both keep more of the history in the chain or cut it at a marginal reading, and what judges the result either way is the contraction gate. That is the source's own buy point, and it is the observation price alone does not supply: on price a rally above an earlier rally top inside a correction and a fresh consolidation under an old peak are the same picture at different magnitudes. A segmentation that neighbouring multiples disagree with is not proposed at all: the disagreement is returned instead. Most real histories come back that way, because real price structure has no single canonical scale, and an unsettled segmentation is the honest answer rather than a defect to tune away.",
            ...
            status_meanings={"ok": "A segmentation the detector will vouch for.", "partial": "Completed price history stops before the requested session.", "unavailable": "No segmentation this capability will stand behind: the history is unusable as bars at all (missing columns, non-numeric values, a repeated session), neighbouring multiples disagree, a session inside the base both extended and reversed a move, or the history segments into no base. The reason names which. Not needs_input, because the parameters are deliberately out of the caller's reach and no argument would change the answer."},
