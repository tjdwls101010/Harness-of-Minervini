...
CAPABILITIES = {
    ...
    for item in [
        ...
        _capability(
            ...
            inputs=_inputs(
                {
                    ...
                    "chain_completeness": _field("enum", "Whether the declared chain is the base's whole structure. A caller may say partial, which admits a gap and costs them nothing. Saying complete has no effect: the reading exists to check the caller's chain, and neither its author nor a word they type can perform that check. It counts only when an independent segmentation supplies it, and until one exists the standard route stops at wait.", choices=["complete", "partial", "needs_chart"]),
                    "entry_price": _field("number", "The price the entry would be taken at. \"At the pivot\" is a claim about a price, so the reading needs one, and the source quantified this distance once -- five, ten, or even twenty cents above the pivot -- which is what the entry is compared against."),
                    ...
                    "entry_proximity": _field("enum", "How close the entry sits to the pivot. The source says to buy as close to the pivot as possible without chasing more than a few percentage points and supplies no number, so how far is too far is the reader's call. What the bars refuse is a price nobody can pay: at_pivot on a pivot price has not cleared, or an entry_price outside the latest completed session's range, which is a past or hoped-for price rather than one on offer. The distance from the pivot, the distance at the breakout, the breakout's age, and Minervini's own stated five-to-twenty-cent buffer are reported for the reader to judge with.", choices=["at_pivot", "chased", "needs_judgment"]),
            ...
            limitations=["Swing segmentation is the caller's chart reading; what the bars can check is that each declared date exists, that the chain alternates and ends on a high, and that each bar really is the extreme of its span.", "Ready requires every item of a route's declared evidence, so a base nobody described is incomplete rather than unobjectionable.", "Three of those items are readings rather than measurements, because completed bars cannot settle a V-shaped right side, whether a chain describes the base's whole structure, or how far above the pivot stops being close to it. What the bars refuse differs by reading. A right side with no pause at all is not constructive. An entry the stock is not trading at today is not an entry, though how far above the pivot stops being close to it is the reader's call: every mechanical rule tried there either invented the number the source withheld or cut somewhere absurd, and using the five-to-twenty-cent band as a ceiling made a band decide a required condition, which its own record forbids. Completeness cannot be self-certified at all and is not a request input, so the standard route stops at wait until an independent segmentation exists to vouch for the chain. The remaining calls -- how close is close, and whether a pivot failure reset promptly -- are the reader's with the measurements printed beside them, and a reader who declares against those numbers is doing so in plain sight.", "All three readings are listed back in declared_readings so the share of the verdict that came from a person is visible, and everything else in it is measured and cannot be declared away.", "Price quieting down on the right side is reported rather than decided: the source says 'noticeably' and gives no number, and a strict comparison of two medians passed a pause two ten-thousandths of a percentage point tighter than its base. The volume half of that same sentence is a gate, because a contraction either happened or it did not.", "Contrast evidence from other practitioners is reported beside the verdict and never enters it.", "The breakout is the first completed close above the pivot after it and is never re-dated, so its volume and closing range are read where the stock actually left the base. Where price stands now is a separate fact: the source says a pivot failure can reset and recover, so a slip below the pivot is counted beside the trigger rather than held against the base, and the trigger reads whether price is above the pivot today.", "A chain declared from a late high is not a misread swing and is not refused; what it leaves above the entry is measured instead and reported as overhead supply.", "The correction the depth limit reads runs from the highest high in the price history the provider returned to the lowest low after it, not from the base the caller declared, and both dates travel with it so a reader can see whether the peak belongs to this base or an older one.", "A cheat entry is not measurable yet: it is entered inside the base rather than at its pivot, so it returns incomplete naming the cheat geometry it needs rather than borrowing the pivot breakout's evidence.", "TL early requires opt-in, explicit confirmation debt, a later pivot, and precise invalidation, and it does not waive the supply gates, which are about the base rather than about when the trade is taken. Its named tactics are not measured yet, so it returns incomplete naming the trigger it needs."],
