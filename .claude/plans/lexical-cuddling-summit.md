...
- **Phase 2 — 설계 리뷰 완료, 5개 슬라이스로 분할.** 설계 리뷰가 blocker 6건으로 구현 착수를 막았고(결정 17~21), 사용자가 세 갈래를 확정했다. 슬라이스 진행 상황은 트래커 #9~#13.
  ...
  - **슬라이스 ④ Power Play — 진행 중** (브랜치 `feat/power-play-evaluator`).
    ...
    - **슬라이스 ④-b로 넘긴 것** (5라운드 확인): 기업행동을 포함하는 Power Play 전용 복합 지문, 차트 승인 seam(승인 대상을 날짜·구간·측정값과 묶기), 추력 내 불연속 관측. **그리고 ④-b의 선결 조건**: 정점을 "구간 최고 High"로 정의하는 한 단조 추력의 직전 봉이 대체 봉우리로 뽑히고(정상 종목도 `disputed`), 미세 신고가가 여럿이면 세 번째 봉우리를 보지 않는다. 해법은 슬라이스 ③의 스윙 검출기로 정점 후보를 스윙 하이로 제한하는 것이다. 지금 유예하는 근거는 공개 경로에 거짓 `qualified`가 없다는 것(거래량 observation이 `pass`를 만들지 않는다)과, 결정 51 이후 분할 컬럼이 있는 실데이터 23종목이 전부 명명된 기준으로 결정적 기각을 낸다는 것이다. **승인 seam을 여는 순간 이 유예는 끝난다.**
