...
23. **체인 분할 게이밍은 슬라이스 ②가 혼자 못 막는다 — 슬라이스 ③이 받는다.** 적대 리뷰가 세 가지 반례를 실행해 보였다: 불리한 수축을 건너뛰어 거친 분할로 재구성하기, 체인을 일찍 끝내 낮은 과거 고점을 피벗으로 삼기, 베이스를 늦게 시작하기.
   - **늦게 시작하기는 슬라이스 ②에서 닫았다.** 깊이 게이트가 이력상 고점과 그 이후 최저점으로 재므로 60% 붕괴 후 늦게 시작한 체인은 10%가 아니라 60%로 읽힌다. 피벗 위 오버헤드 서플라이도 함께 보고된다.
   - **건너뛰기·병합은 못 닫는다.** 줄인 체인의 앵커들은 각자 구간의 진짜 극값이고, 측정된 깊이는 그 분할을 정직하게 서술한다. 문제는 호출자가 유리한 분할을 고를 수 있다는 것이고, 이건 **독립적인 정준 분할이 없으면 원리적으로 판정 불가**다. 그 분할을 만드는 것이 슬라이스 ③의 검출기다. 검출기 분할과 호출자 체인이 실질적으로 어긋나면 그 불일치 자체가 보고돼야 하고, 결정 19에 따라 검출기 단독은 WAIT까지이므로 두 분할이 서로를 견제하는 구조가 된다.
   - **낮은 피벗도 같은 뿌리다.** 마지막 고점의 오른쪽 이웃이 자기 자신이라 상승 도중의 아무 중간 고점이나 피벗이 될 수 있다. 검출기가 최종 pause를 독립적으로 제시하면 대조할 대상이 생긴다.

24. **오래되고 과도하게 연장된 돌파는 보고하되 거부하지 않는다.** 40세션 전에 돌파해 지금 50% 연장된 셋업도 READY다. 원전의 추격 한계는 "without chasing the stock up more than a few percentage points"이고 **숫자가 없다.** 임계를 발명하는 대신 세 가지를 함께 싣는다 — 돌파 시점의 연장률, 현재 연장률, 돌파 이후 경과 세션 수. 응답 표준이 marker 측정치를 값과 거리까지 보고하라고 요구하므로 독자가 판단할 재료는 전부 간다. TraderLion의 1.5% 존은 비구속 게이트로 배선할 수 있으나 슬라이스 ⑤ 몫이다.
