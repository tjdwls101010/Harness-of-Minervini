...
`tests/e2e/scenarios.json` is the catalog: <<N>> prompt families, each with `critical_assertions` and `noncritical_assertions`. `tests/e2e/reports/<scenario_id>/run-{1,2,3}.json` holds <<N*3>> reports. Each report carries the analyst's `final_response`, the `commands` it ran, and its own self-scoring of every assertion with `evidence`.
